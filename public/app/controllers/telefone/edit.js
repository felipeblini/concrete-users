angular.module('myApp').controller('EditTelefoneController', 
  ['$scope', '$routeParams', '$location', '$translate', '$timeout', 'UserErrorService', 'NavigationService', 'EntityUtilService', 'SecurityService', 'TelefoneService', 
  function($scope, $routeParams, $location, $translate, $timeout, UserErrorService, NavigationService, EntityUtilService, SecurityService, TelefoneService) {
	...
	$scope.add = function () {
		...
		TelefoneService.add(dataToServer($scope.obj)).then(function (httpResponse) {
			if($scope.parent) {
	...
	$scope.update = function () {
		$scope.uiWorking = true;
		TelefoneService.update(dataToServer($scope.obj)).then(function () { //httpResponse
				returnBack();
	...
	$scope.delete = function () {
		$scope.uiWorking = true;
		TelefoneService.delete($scope.obj._id).then(returnBack, errorHandlerDelete, progressNotify);
	};
	...
	$scope.gotoEdit = function() {
		$location.path('/telefone/edit/' + $routeParams.id);		
	};

	$scope.gotoDelete = function() {
		$location.path('/telefone/delete/' + $routeParams.id);		
	};
	...
	function init() {
		...
		if ($routeParams.id) {
			TelefoneService.getDocument($routeParams.id).then(loadData, errorHandlerLoad);		
		}
